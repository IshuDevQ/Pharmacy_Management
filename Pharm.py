"""
PHARMACY MANAGEMENT (Tkinter + MySQL) — Updated

Fixes requested now:
1) ✅ HOME DELIVERY alignment fixed (clean grid, no cut-off buttons, proper spacing)
2) ✅ PRINT INVOICE: added option to view previous invoices
   - Dropdown (last 20 orders by default) + Load Selected + Load Last

Keeps:
- Sales analysis removed
- Show order list has CLEAR
- DB schema auto-migration for qty/line_total
"""
...
def open_home_delivery_window(root):
    """
    Alignment fixed:
    - Left form is a grid with consistent row heights
    - Right panel uses grid; cart and output expand properly
    - Bottom buttons aligned in a single row and not clipped
    """
    w = tk.Toplevel(root)
